...
while($url = array_pop($category_urls)){
	...
	if( preg_match('|(http://category\.suning\.com/cid.*)\.html|', $url, $url_info) ){
		//获取最大链接
	...
	}elseif( preg_match('|http://category\.suning\.com/all/\?category_path=*|', $url) ){
		if(preg_match('|<span>共([0-9]*)页 到第</span>|isU', $content, $page_info)){
			$max_page = intval($page_info[1]);
		}
		//print_r($url);
	...
	}elseif( preg_match('|(http://e\.suning\.com/list_.*)\.htm|', $url, $url_info) ){
...
function category_urls($content){
	...
	foreach($urls as $index => $url){
		...
		if( !preg_match('|http://category.newegg.com.cn/all/\?category_path=*|', $url)
		 && !preg_match('|http://e.newegg.com.cn/list_*|', $url)
		 && !preg_match('|http://category.newegg.com.cn/cid*|', $url)
		){
			print_r($url);echo "\n";
			unset($urls[$index]);
			continue;
...
while ($url = array_pop($uncrawled_urls)){
	...
	foreach ($links as $link){
		if(!in_array($link, $crawled_urls) && !in_array($link, $uncrawled_urls)){
			if(stripos($link, 'newegg.com.cn') === false || stripos($link, 'comment') !== false){
				continue;
			}
